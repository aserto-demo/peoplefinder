...
exports.register = (app) => {
  app.get("/api/users", checkAuthz(`${applicationName}.users.get`, ''), (req, res) => {
    (async () => { res.status(200).send(await directory.getUsers(req)) })();
  });

  app.get("/api/users/:id", checkAuthz(`${applicationName}.users.__id.get`, 'id'), (req, res) => {
    const id = req.params.id;
    (async () => { res.status(201).send(await directory.getUser(req, id)) })();
  });  

  // edit name and email
  app.put("/api/users/:id", checkAuthz(`${applicationName}.users.__id.put`, 'id'), (req, res) => {
    const user = req.body;
    const id = req.params.id;
  ...
  app.post("/api/users/:id", checkAuthz(`${applicationName}.users.__id.post`, 'id'), (req, res) => {
    const user = req.body;
    const id = req.params.id;
    (async () => { res.status(201).send(await directory.updateUser(req, id, user)) })();
  });  

  app.delete("/api/users/:id", checkAuthz(`${applicationName}.users.__id.delete`, 'id'), (req, res) => {
    const user = req.body;
    const id = req.params.id;
